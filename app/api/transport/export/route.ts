...
function buildFileName() {
  ...
  return `Transportbericht_${date}_${time}.xlsx`;
}
...
export async function POST(request: NextRequest) {
  ...
  const flowUrl = process.env.POWER_AUTOMATE_TRANSPORT_EXPORT_URL?.trim();
  if (!flowUrl) {
    return NextResponse.json(
      { message: "POWER_AUTOMATE_TRANSPORT_EXPORT_URL ist nicht konfiguriert." },
      { status: 501 }
    );
  }
