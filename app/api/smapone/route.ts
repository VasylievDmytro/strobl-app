...
import { getSmapOneAnalytics } from "@/lib/dataverse/service";
...
export async function GET(request: NextRequest) {
  ...
  const bauleiter = request.nextUrl.searchParams.get("bauleiter") || undefined;
  const projectNumbers = parseProjectNumbers(request.nextUrl.searchParams.get("projects"));

  const analytics = await getSmapOneAnalytics({
    month,
    date,
    periodMode,
    employeeName,
