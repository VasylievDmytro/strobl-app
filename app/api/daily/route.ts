...
import { getDailyFilterOptions, getDailyReports } from "@/lib/dataverse/service";
...
export async function GET(request: NextRequest) {
  ...
  const hasSearchFilter = params.getAll("lv").length > 0 || Boolean(params.get("reportType"));

  const filters = {
    dateFrom:
      params.get("dateFrom") ||
      (!hasExplicitDateFilter && !hasSearchFilter ? defaultRange.dateFrom : undefined),
    dateTo:
      params.get("dateTo") ||
      (!hasExplicitDateFilter && !hasSearchFilter ? defaultRange.dateTo : undefined),
  ...
  const [data, options] = await Promise.all([
    ...
    getDailyFilterOptions({ bauleiter: access.isAdmin ? undefined : access.bauleiter })
  ]);
