...
interface SmapOneResponse {
  analytics: SmapOneAnalytics;
  access: UserAccessScope;
}

function normalizeAnalytics(analytics: SmapOneAnalytics): SmapOneAnalytics {
  return {
    ...analytics,
    ...
    sourceLeaderboard: analytics.sourceLeaderboard ?? [],
    monthlyTrend: analytics.monthlyTrend ?? []
  };
}

function normalizeResponse(payload: SmapOneResponse): SmapOneResponse {
  return {
...
export function SmapOneDashboard() {
  ...
  const [response, setResponse] = useState<SmapOneResponse | null>(null);
  ...
  useEffect(() => {
    ...
    if (selectedBauleiter) {
      params.set("bauleiter", selectedBauleiter);
    }
    ...
    fetchJson<SmapOneResponse>(`/api/smapone?${params.toString()}`)
      .then((payload) => setResponse(normalizeResponse(payload)))
      .catch(() => {
        setResponse(null);
        setError("Die SmapOne-Auswertung konnte nicht geladen werden.");
      })
      .finally(() => setLoading(false));
  }, [periodMode, projectNumbers, selectedBauleiter, selectedDate, selectedEmployee, selectedMonth]);
  ...
  useEffect(() => {
    ...
    if (analytics && !analytics.availableBauleiter.includes(selectedBauleiter)) {
      setSelectedBauleiter("");
    }
  }, [analytics, selectedBauleiter]);
  ...
  return (
    <div className="space-y-6">
      <PageTitle
        eyebrow="Arbeitszeit"
        title="SmapOne Analyse"
        description="Monatliche Auswertung der erfassten Mitarbeiterstunden aus Tagesbericht und Transportbericht mit identischen Filtern wie in GeoCapture."
      />

      <FilterBar className="xl:grid-cols-[0.42fr_0.5fr_0.7fr_0.75fr_1fr_auto_1.1fr]">
        ...
        <label className="space-y-2">
          ...
          <select
            ...
            onChange={(event) => setSelectedBauleiter(event.target.value)}
          >
            <option value="">Alle</option>
            {(analytics?.availableBauleiter ?? []).map((bauleiter) => (
              <option key={bauleiter} value={bauleiter}>
                {bauleiter}
              </option>
            ))}
          </select>
        ...
        <div className="surface-muted flex min-h-[88px] items-center justify-between gap-4 px-4 py-4">
          <div>
            ...
            <p className="mt-1 text-sm text-ink-500">
              {access?.isAdmin
                ? `Die Kennzahlen kombinieren Tagesbericht und Transportbericht für ${analytics?.selectedLabel ?? "den ausgewählten Zeitraum"}.`
        ...
        <>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            ...
            <InfoCard
              ...
              hint="Durchschnitt auf Basis aller aktiven Mitarbeiter im Zeitraum."
              valueClassName="text-[clamp(1.4rem,4vw,2.3rem)]"
            />
            <InfoCard
              ...
              hint={
                analytics.topEmployee
                  ...
                  : "Im ausgewählten Zeitraum wurden keine Stunden erfasst."
              }
          ...
          {analytics.entryCount === 0 ? (
            <EmptyState
              title="Keine SmapOne-Daten im ausgewählten Zeitraum"
              description="Für diesen Zeitraum wurden aus Tagesbericht und Transportbericht keine passenden Arbeitszeiten gefunden."
            />
          ) : (
            <>
              <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
                ...
                <AnalyticsPanel
                  ...
                  description="Die stärksten Mitarbeiterwerte aus beiden Systemen im direkten Vergleich."
                  icon={Crown}
                >
              ...
              <div className="grid gap-4 xl:grid-cols-3">
                <AnalyticsPanel
                  eyebrow="Bauleiter"
                  title="Stunden nach Bauleiter"
                  description="Welche Bauleiter im ausgewählten Zeitraum die meisten Mitarbeiterstunden gebunden haben."
                  icon={Building2}
                >
                  <RankingList items={analytics.bauleiterLeaderboard} accent="blue" />
                </AnalyticsPanel>

                <AnalyticsPanel
                  eyebrow="Projekte"
                  title="Stunden nach Projekt"
                  description="Top-Projekte beziehungsweise LV-Nummern nach gebuchten Stunden."
                  icon={CalendarRange}
                >
                ...
                <AnalyticsPanel
                  eyebrow="Quellen"
                  title="Stunden nach Berichtsquelle"
                  description="Vergleich von Tagesbericht und Transportbericht als parallele Zeiterfassungssysteme."
                  icon={Layers3}
                >
                  <RankingList items={analytics.sourceLeaderboard} accent="emerald" />
                </AnalyticsPanel>
              </div>
            </>
          )}
  ...
  if (!items.length) {
    return (
      <EmptyState
        ...
        description="Für diese Auswertung wurden im ausgewählten Zeitraum keine Werte gefunden."
